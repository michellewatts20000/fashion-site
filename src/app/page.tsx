import Image from "next/image";

import BestGrid from "./components/BestGrid";
import Button from "./components/Button";
import SectionHeading from "./components/SectionHeading";
import Designer from "./components/Designer";

import localFont from "next/font/local";
const myFont = localFont({
  src: "/ui/Elephant-Regular.woff2",
});

import allProducts from "@/app/data/product.js";

const { products, products2, products3 } = allProducts;

export default function Home() {
  return (
    <main className="max-w-[1920px] mx-auto">
      <section className="bg-limeGreen w-full hero relative">
        <nav className="top-[54px] absolute w-full">
          <div className="flex w-full items-center justify-between px-[175px]">
            <div className={`${myFont.className} text-40 text-darkGreen`}>
              Rivo
            </div>
            <div className="ml-40">
              <ul className="flex gap-10 nav-items">
                <li>Home</li>
                <li>Shop</li>
                <li>Features</li>
                <li>Contact</li>
              </ul>
            </div>
            <div className="flex items-center justify-center gap-10">
              <Image
                src={"/shopping-icon.png"}
                width={32}
                height={32}
                alt={"shopping icon"}
                className="mt-2"
                priority
                style={{ width: 32, height: 32 }}
              />
              <button className="drop-shadow-lg w-[142px] h-[52px] outline-darkGreen border-2 border-darkGreen text-20 uppercase font-poppins font-medium">
                Login
              </button>
            </div>
          </div>
        </nav>
        <div className="flex w-full px-[175px] min-h-[1100px] h-screen justify-center items-center">
          <div className="w-1/2">
            <h1 className="text-clampHeading text-darkGreen font-rufina mb-6 font-bold leading-normal">
              Discover and <br /> Find Your Own Fashion!
            </h1>
            <p className="max-w-[565px] subheading">
              Explore our curated collection of stylish clothing and accessories
              tailored to your unique taste.
            </p>
            <button className="drop-shadow-2xl bg-darkGreen text-white px-6 py-4 mt-10 font-poppins text-20 w-[258px] h-[74px] font-medium tracking-wider">
              EXPLORE NOW
            </button>
          </div>
          <div className="w-1/2">
            <Image
              src={"/hero.png"}
              width={570}
              height={717}
              alt={"hero"}
              priority={true}
              className="ml-auto"
            />
          </div>
        </div>
      </section>
      <section className="w-full max-w-[1377px] mx-auto">
        <SectionHeading
          title="Best selling"
          description="Get in on the trend with our curated selection of best-selling styles."
        />
        <div className="flex items-start justify-center gap-12">
          {products.map((p) => (
            <BestGrid
              key={p.id}
              image={p.image}
              productTitle={p.productTitle}
              stars={p.stars}
              price={p.price}
              width={430}
              height={566}
            />
          ))}
        </div>
        <div className="mx-auto w-full text-center flex flex-col items-center">
          <Button text="See All" />
        </div>
      </section>
      <section className="w-full max-w-[1377px] mx-auto">
        <SectionHeading
          title="Our Products"
          description="Get in on the trend with our curated selection of best-selling styles."
        />
        <div className="flex items-start justify-center gap-6 flex-wrap">
          {products2.map((p) => (
            <BestGrid
              key={p.id}
              image={p.image}
              productTitle={p.productTitle}
              stars={p.stars}
              price={p.price}
              width={318}
              height={378}
            />
          ))}
        </div>
      </section>
      <section className="max-w-[1377px] mx-auto py-20">
        <div className="bg-limeGreen w-full h-[596px] overflow-hidden relative">
          <Image
            src={"/breakout.png"}
            width={482}
            height={596}
            alt={"breakout"}
            className="absolute bottom-0 left-20"
          />
          <div className="flex items-center justify-end h-full">
            <div className="w-1/2">
              <div className="mb-10">
                <h1 className="text-46 text-darkGreen font-roboto font-semibold mb-4">
                  Exclusive offer
                </h1>
                <p className="leading-custom max-w-[590px]">
                  Unlock the ultimate style upgrade with our exclusive offer
                  Enjoy savings of up to 40% off on our latest New Arrivals
                </p>
              </div>
              <div className="flex gap-4">
                <div className="bg-white w-[100px] h-[100px] flex items-center justify-center">
                  <div className="text-center">
                    <p className="text-darkGreen leading-tight text-32">06</p>
                    <p className="text-darkGreen text-16 font-light">Days</p>
                  </div>
                </div>
                <div className="bg-white w-[100px] h-[100px] flex items-center justify-center">
                  <div className="text-center">
                    <p className="text-darkGreen leading-tight text-32">18</p>
                    <p className="text-darkGreen text-16 font-light">Hours</p>
                  </div>
                </div>
                <div className="bg-white w-[100px] h-[100px] flex items-center justify-center">
                  <div className="text-center">
                    <p className="text-darkGreen leading-tight text-32">48</p>
                    <p className="text-darkGreen text-16 font-light">Minutes</p>
                  </div>
                </div>
              </div>
              <button className="drop-shadow-2xl bg-darkGreen text-white px-6 py-4 mt-10 font-poppins text-20 w-[235px] h-[74px] font-medium tracking-wider">
                BUY NOW
              </button>
            </div>
          </div>
        </div>
      </section>
      <section className="max-w-[1377px] mx-auto pb-10">
        <SectionHeading
          title="Designer Clothes For You"
          description="Immerse yourself in the world of luxury fashion with our meticulously crafted designer clothes!"
        />
        <div className="flex gap-10">
          {products3.map((p) => (
            <Designer
              key={p.id}
              image={p.image}
              heading={p.heading}
              description={p.description}
              width={430}
              height={566}
            />
          ))}
        </div>
      </section>
    </main>
  );
}
